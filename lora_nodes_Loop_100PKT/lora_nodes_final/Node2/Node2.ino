...
//#define QUICK_DEBUG
#ifdef QUICK_DEBUG
  const unsigned long WATCHDOG_TIMEOUT = 46000UL;
  const unsigned long STATUS_PRINT     = 20000UL;
  const unsigned long PAUSE_BETWEEN_TESTS = 56000UL;  // Must match Node 1
#else
  const unsigned long WATCHDOG_TIMEOUT = 91000UL;
  const unsigned long STATUS_PRINT     = 300000UL;
  const unsigned long PAUSE_BETWEEN_TESTS = 56000UL;
#endif
...
void setup() {
  ...
  while (!Serial); 
  delay(500);
  Serial.println(F("\nNode 2 - Forwarder (with LAST flag sync)"));
...
void loop() {
  ...
  if (sz > 15) {
    ...
    // Parse: test1_SF7_CR5|ID:1|num:012|LAST|NEXT:test2_SF7_CR8
    char testName[30];
    int fromID, pktNum;
    char nextTestName[30] = "";
    bool isLast = false;
    
    // Check if LAST flag is present
    if (strstr(buf, "|LAST|")) {
      isLast = true;
      
      // Extract next test name
    ...
    // Detect test change (for non-LAST packets)
    int newTest = detectTest(testName);
    
    // Get RSSI/SNR before forwarding
    ...
    // Handle LAST packet - pre-configure for next test
    if (isLast && strlen(nextTestName) > 0) {
      int nextTest = detectTest(nextTestName);
      if (nextTest >= 0) {
        Serial.print(F("LAST packet! Waiting ")); 
        Serial.print(PAUSE_BETWEEN_TESTS/1000);
        Serial.println(F("s then switching..."));
        
        // Wait for Node 1's pause duration
        delay(PAUSE_BETWEEN_TESTS);
        
        currentTest = nextTest;
        setConfig(currentTest);
        Serial.println(F("Ready for next test!"));
      }
    } 
    // Handle regular test change (fallback for missed LAST)
    else if (newTest >= 0 && newTest != currentTest) {
      currentTest = newTest;
      setConfig(currentTest);
    }
    ...
    Serial.print(F("FWD → ")); 
    Serial.print(out);
    if (isLast) Serial.print(F(" [LAST PKT]"));
    Serial.print(F(" RSSI:")); 
    Serial.print(rssi);
    Serial.print(F(" SNR:")); 
    Serial.println(snr);
  ...
  // Watchdog + scan mode (as failsafe)
  if (millis() - lastRx > WATCHDOG_TIMEOUT) {
    unsigned long silenceDuration = millis() - lastRx;
    
    /*if (silenceDuration < WATCHDOG_TIMEOUT * 1.5) {
      reinitLoRa();
      lastRx = millis() - (WATCHDOG_TIMEOUT / 2);
    } else {
      Serial.print(F("No packets for "));
      Serial.print(silenceDuration/1000);
      Serial.println(F("s → SCANNING"));
      */
      Serial.println(F("s → SCANNING"));
      scanMode();
      lastRx = millis();
  ...
  //if (millis() - lastStatus > STATUS_PRINT) {
    //Serial.print(F("Status: Test ")); 
   // Serial.print(currentTest+1);
   // Serial.print(F(", LastRx: ")); 
   // Serial.print((millis() - lastRx)/1000);
